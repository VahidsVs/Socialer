    ...
    <div class="container py-md-5 container--narrow">
        <h2>
          ...
          @auth
            ...
            <form class="ml-2 d-inline" action="/delete-follow/{{ $sharedData["username"] }}" method="POST">
              @csrf
            <button class="btn btn-danger btn-sm">Unfollow <i class="fas fa-user-plus"></i></button>
          </form>
            @endif
            @if (auth()->user()->username == $sharedData["username"])
            <a href="/view-manage-avatar" class="btn btn-secondary btn-sm">Manage Avatar</a>
            @endif
        ...
        <div class="profile-nav nav nav-tabs pt-2 mb-4">
          ...
          <a href="/profile/{{$sharedData["username"]}}/followings" class="profile-nav-link nav-item nav-link {{ Request::segment(3)=="followings" ? "active" : "" }}">Followings: {{$sharedData["followingCount"] }}</a>
        </div>
